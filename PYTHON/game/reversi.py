...
class Reversi:
    ...
    def play_game(self):
        ...
        while self.winner(state) is False:
            color = state[1]
            picked = self.agent_pick_move(state, self.lastAction) #, self.lastAction ###
            # Save then action so can be used in HBA
            self.lastAction = picked
    ...
    def agent_pick_move(self, state, lastAction): #, lastAction ###
        color = state[1]
        legal_moves = self.legal_moves(state)
        picked = None
        # The following assumes HBA is white White
        if color == WHITE:
            #self.black_agent.workspace.update_opp_strategies(state, legal_moves)
            picked = self.white_agent.get_action(state, legal_moves, lastAction, self.t) # , lastAction, self.t ###
            #self.lastAction = picked
        elif color == BLACK:
            self.white_agent.workspace.update_opp_strategies(state, legal_moves) # What would each type do? ###
            picked = self.black_agent.get_action(state, legal_moves) # , lastAction, self.t ###
            self.lastAction = picked ###
        else:
            raise ValueError
